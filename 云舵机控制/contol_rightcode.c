...
//改1
#define TRAFFIC_LIGHT_CMD_CONTROL_MODE      "ServoControl"

#define TRAFFIC_LIGHT_CMD_AUTO_MODE         "AutoModule"
#define TRAFFIC_LIGHT_CMD_HUMAN_MODE        "HumanModule"

//改2
#define TRAFFIC_LIGHT_YELLOW_ON_PAYLOAD     "STOP"  //yellow->stop
#define TRAFFIC_LIGHT_RED_ON_PAYLOAD        "ROUND_FAST" //red->round
#define TRAFFIC_LIGHT_GREEN_ON_PAYLOAD      "ROUND_SLOW"
#define TRAFFIC_LIGHT_SERVICE_ID_PAYLOAD    "turn"
...
//根据传入的模式（appOptionMode）和控制类型（appOptionType）来更新当前交通灯的状态，并报告当前状态。
...
//舵机控制上下移动函数
void ControlUP(int high_t,int t)
{
    for(int i=0;i<t;i++){
        ...
        hi_udelay(20000-high_t); 
    }
}

//舵机控制旋转函数
void ControlTurn(int high_t,int t)
{
    for(int i=0;i<t;i++){
        IoTGpioSetOutputVal(IOT_GPIO_INDEX_11,1);
        hi_udelay(high_t);
...
static void TrafficLightMsgRcvCallBack(char *payload)
{
    ...
    if (strstr(payload, TRAFFIC_LIGHT_CMD_CONTROL_MODE) != NULL) {
        ...
        if (strstr(payload, TYPE1) != NULL) { // YELLOW LED->stop
            OledShowStr(OLED_X_POSITION_0, OLED_Y_POSITION_7, "2.stop     ",
        ...
        } else if (strstr(payload, TYPE2) != NULL) { // RED LED->round
            ...
            //             IOT_GPIO_DIR_OUT, IOT_GPIO_VALUE0, IO_FUNC_GPIO_OUT);
        } else if (strstr(payload, TYPE) != NULL) { // GREEN LED
...
// {   int high_t;
//     if(dir==-1) high_t=2300;
//     if(dir==0) high_t=1500;        //stop
//     if(dir==1) high_t=500;        //                                       
//     circle(high_t,40);
// }
